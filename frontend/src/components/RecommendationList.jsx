...
function RecommendationList({ recommendations, preferences, onReset, onLogout, user }) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 mb-2">Your Personalized Recommendations</h1>
              <p className="text-slate-600">
                Based on your preferences, here are the top 3 products we found for you
              </p>
            </div>
        ...
        {/* Preferences Summary */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Your Search Criteria</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-slate-500">Categories:</span>
              <p className="font-semibold text-slate-900">{preferences.interests.join(', ')}</p>
            </div>
            <div>
              <span className="text-slate-500">Budget:</span>
              <p className="font-semibold text-slate-900">${preferences.budget}</p>
            </div>
            <div>
              <span className="text-slate-500">Brand:</span>
              <p className="font-semibold text-slate-900">{preferences.brandPreference}</p>
            </div>
            <div>
              <span className="text-slate-500">Feature:</span>
              <p className="font-semibold text-slate-900">{preferences.featurePreference}</p>
            </div>
          </div>
        </div>
                ...
                {explanations[product.id] && (
                  <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-slate-800 mb-1">Why this matches you</p>
                        <p className="text-sm text-slate-700">{explanations[product.id]}</p>
                      </div>
                    </div>
                  </div>
                )}
